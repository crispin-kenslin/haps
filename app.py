...
# Database model to record unique views by calendar day
...
def get_visitor_stats():
    """Fetches or creates today's log and aggregates total visits."""
    today = date.today()
    today_record = VisitorLog.query.filter_by(date=today).first()
    if not today_record:
        today_record = VisitorLog(date=today, count=0)
        db.session.add(today_record)
        db.session.commit()
    
    total_visitors = db.session.query(db.func.sum(VisitorLog.count)).scalar() or 0
    return today_record, total_visitors
# --------------------------------------
...
def index():
    ...
    # Session handling logic to prevent multi-count on browser refresh
    ...
    return render_template('index.html', daily_count=today_record.count, total_count=total_visitors)
...
if __name__ == '__main__':
    # Initialize the tracking database tables automatically before starting
